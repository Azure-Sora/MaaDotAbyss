...
- 🤖 **会自己变快的日常任务** —— 初次由 LLM 看图完成；重复成功的安全点击轨迹会蒸馏为 shadow 剧本，连续快跑验证后转 trusted，失败自动降级给 LLM 修复。稳定任务不再逐步调用模型
...
- `.local/evolution.json` 保存各任务 observing/shadow/trusted/degraded 的演进证据；候选程序位于 `tasks/flows/`，可以直接审阅和版本管理
